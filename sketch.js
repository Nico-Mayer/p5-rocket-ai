...
let editModeControls = document.getElementById('editModeControls')
let statTracker = document.getElementById('statTracker')
	...
	if (editMode) {
		...
		for (var i = 0; i < tempArr.length; i++) {
			...
			if (mouseIsPressed && tempArr[i].rezisable && resizeMode) {
				tempArr[i].width = constrain(
					mouseX - tempArr[i].x,
					20,
					canWidth
				)
				tempArr[i].height = constrain(
					mouseY - tempArr[i].y,
					20,
					canHeight
				)
...
function setupInfos() {
	...
	editBtn.onclick = function () {
		...
		statTracker.classList.toggle('hidden')
